...
class EnhancedSmartMoneyScreener:
    ...
    def calculate_composite_score(self, row: pd.Series, tech: Dict, fund: Dict, analyst: Dict, rs: Dict) -> Tuple[float, str]:
        ...
        # Determine grade
        if composite >= 80: grade = "S grade (immediate buy)"
        elif composite >= 70: grade = "A grade (strong buy)"
        elif composite >= 60: grade = "B grade (consider buy)"
        elif composite >= 50: grade = "C grade (watch)"
        elif composite >= 40: grade = "D grade (caution)"
